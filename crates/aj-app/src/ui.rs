...
use aj_core::{BlendMode, BrushParams, BrushType, HistoryStatus, LayerId, LinearRgba, Page};
...
/// Layer-related actions. Emitted by the layers panel; dispatched into the
/// engine as `Command`s. `AddLayer`'s name is generated by the UI (the
/// engine is naming-agnostic) so users can rename later without round-
/// tripping a name generator through `Command`. Add / Remove / Move are
/// undoable; the rest are non-undoable today (matches `SetPageSize` /
/// `SetBrushColor` precedent — the engine has a `TODO(undoable-layer-edits)`).
...
pub enum LayerAction {
    ...
    SetLayerVisible { id: LayerId, visible: bool },
    SetLayerOpacity { id: LayerId, opacity: f32 },
    SetLayerBlendMode { id: LayerId, mode: BlendMode },
}

impl LayerAction {
    pub fn dispatch(self, engine: &Engine) {
        let cmd = match self {
            Self::AddLayer { name } => Command::AddLayer { name },
            Self::RemoveLayer(id) => Command::RemoveLayer { id },
            Self::MoveLayer { id, new_index } => Command::MoveLayer { id, new_index },
            Self::SetActiveLayer(id) => Command::SetActiveLayer { id },
            Self::SetLayerVisible { id, visible } => Command::SetLayerVisible { id, visible },
            Self::SetLayerOpacity { id, opacity } => Command::SetLayerOpacity { id, opacity },
            Self::SetLayerBlendMode { id, mode } => Command::SetLayerBlendMode { id, mode },
        };
        engine.send(cmd);
    }
}
