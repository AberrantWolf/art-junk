...
impl StylusAdapter {
    ...
    // --- Android MotionEvent seam ---
    //
    // Android delivers fully-resolved axis samples per `MotionEvent`. No
    // Estimated/Revise cycle — the backend walks `getHistoricalAxisValue`
    // plus the current sample, emitting `AndroidRawSample`s in time order.
    // Tilt decomposition happens in the adapter (platform-agnostic math)
    // so the backend stays thin.
...
mod tests {
    ...
    // --- Android MotionEvent seam tests ---
